%% Does the wave change shape with x? Aproximate shape


%Start at time 10 ~ Steady state
x_positions = 0.3:0.001:0.7;

figure(2)
leg = {}; count = 0;
% Find 3 osculations after 10 seconds and plot them
for i = find(x >= x_positions(1),1):find(x >= 0.001,1):find(x >= x_positions(end),1)
    count = count+1;
    start_t = find(t>=20,1); % Start at 10 seconds
...
%     subplot(2,3, count)
    hold off
    plot([t_inQ(find(max(Z_inQ)==Z_inQ)), t_inQ(find(max(Z_inQ)==Z_inQ))], [0.2, max(Z_inQ)],':b', 'linewidth', 2)
    hold on
...
%     grid on
    ...
    %% Plot Quartiles
    Quartiles_wanted = [0.05, 0.25,0.5,0.75];
    x_QuartilesFB(count,1) = x(i); 
    for j = 1:length(Quartiles_wanted)
        frount_Q = find(Z_inQ>Quartiles_wanted(j)*max(Z_inQ),1,'last');
        back_Q = find(Z_inQ>Quartiles_wanted(j)*max(Z_inQ),1);
        x_QuartilesFB(count,j+1) = t_inQ(frount_Q)- t_inQ(find(max(Z_inQ)==Z_inQ));
        x_QuartilesFB(count,j+5) = t_inQ(back_Q)- t_inQ(find(max(Z_inQ)==Z_inQ));
    end
    m_hindx = find(max(Z_inQ)==Z_inQ);
    a_p_f = (t_inQ(end)-t_inQ(m_hindx))*(max(Z_inQ)-min(Z_inQ));
    a_p_b = (t_inQ(m_hindx)- t_inQ(1))*(max(Z_inQ)-min(Z_inQ));
    x_QuartilesFB(count,11) = trapz(t_inQ(1:m_hindx), Z_inQ(1:m_hindx)-min(Z_inQ))/a_p_b ;
    x_QuartilesFB(count,10) = trapz(t_inQ(m_hindx:end), Z_inQ(m_hindx:end)-min(Z_inQ))/a_p_f;
...
subplot(1,2,1)
for i = 2:9
hold on; plot(x_QuartilesFB(:, 1), x_QuartilesFB(:, i),mcolours(mod(i-2,4)+1))
end
grid on
title('Wave countour over x')

subplot(2,2,2)
for i = 1:4
    pFH = (x_QuartilesFB(:, i+1) + x_QuartilesFB(:, i+5))./x_QuartilesFB(:, i+1);
    hold on;
    plot(x_QuartilesFB(:, 1), pFH, mcolours(i))
end
legend('0.05 of max', '0.25 of max', '0.5 of max', '0.75 of max')
grid on; title('Percentage Front Heavy')

subplot(2,2,4)
for i = 1:4
    FH = (x_QuartilesFB(:, i+1) + x_QuartilesFB(:, i+5));
    hold on;
    plot(x_QuartilesFB(:, 1), FH, mcolours(i))
end
legend('0.05 of max', '0.25 of max', '0.5 of max', '0.75 of max')
grid on; title('Percentage Front Heavy')

subplot(2,2,4)
FH = x_QuartilesFB(:, 4)./(x_QuartilesFB(:, 2) - x_QuartilesFB(:, 6));
BH = x_QuartilesFB(:, 8)./(x_QuartilesFB(:, 2) - x_QuartilesFB(:, 6));
hold on;
plot(x_QuartilesFB(:, 1), FH); plot(x_QuartilesFB(:, 1), BH)
legend('Front heavy', 'Back heavy')
grid on; title('Percentage Front Heavy')

figure(7)
hold on
plot(x_QuartilesFB(:,1), x_QuartilesFB(:,10), 'g', x_QuartilesFB(:,1), x_QuartilesFB(:,11), 'k')
grid on; xlabel('Position, x(beta)'); ylabel('Percentage area')
