...
figure(1)
subplot(2,3,1)
        imagesc(t,flipud(x),Z0D) % (:, 1:20/dt+1)
        set(gca,'YDir','normal')
        xlabel('Time, [s]')
        ylabel('Position, x')
...
y0 = [Z_0, V_0, Y_0];
for i = 1:length(x)
    mybeta_temp = i + d2Zdx2(i,:)/v_1;%(i,1:find(t>t_end_first(i),1));
    plot(mybeta_temp)
    pause(0.1)
...
Z_new = y0D_pD(:, 1:M)';
V_new = y0D_pD(:, M+1:2*M)';
Y_new = y0D_pD(:, 2*M+1:3*M)';
    
subplot(2,3,4)
        hold on
        max_line_0D = max(Z0D(:, 20/dt:end), [], 2);
        min_line_0D = min(Z0D(:, 20/dt:end), [], 2);
...
subplot(2,1,2)
        imagesc(t,flipud(x),Z_new) % (:, 1:20/dt+1)
        set(gca,'YDir','normal')
        xlabel('Time, [s]')
        ylabel('Position, x')
        title(['[Ca^2^+]_{Cytosol} zero Diffusion, [\mu M]'])
