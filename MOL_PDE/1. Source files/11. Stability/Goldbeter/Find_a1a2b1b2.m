...
possible_D = [0, 2,4,6,8]*1e-6;

DepthResults = [];
firstwave = [];
count = 0;
for kk = 1:length(possible_D)
    display(['Goldbeter', num2str(kk)])
    Dupont.On = 1;
    ...
    if D ~= 0
        for jj = 1:9
            [ wave_data ] = Follow_wave( ZFD, x, t, 0.54, jj, [1, 1] );
...
for ii = 0:0.001:0.2530
    Rate =  ii;
    [ Perts_dbdx, Plot_dbetadx, plotxer ] = ItterationDownBeta( perts, betas_coll, plotted_Z, betas_coll, betas_coll, Rate, 0.2 );
