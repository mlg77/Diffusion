function [ pathdata ] = ItterationDownBeta_height( Peribtations, Betas, AoPGrid, newBetaVector, f_1, IP )
...
%   Outputs:
%       pathdata.E      = E of path
%       pathdata.P      = P of path
%       pathdata.x      = x of path
%       (OLD)   Perts           = vertor length d
%       (OLD)   PlotBetaVctor   = vertor length d 
...
%     AoP = AoPGrid(beta_x, idx_0_per); % Finds aop value of x*
    ...
    Pert_x = P_x_min_1;
%     beta_x = find(Betas >= newBetaVector(end-ii), 1);
    beta_x = length(newBetaVector)-ii;
    vq = interp1(Peribtations',AoPGrid',Pert_x);
    AoP = vq(beta_x);
...
pathdata.E = to_plot(1:end_depth-1, 2);
pathdata.P = Perts;
pathdata.x = xvec;
