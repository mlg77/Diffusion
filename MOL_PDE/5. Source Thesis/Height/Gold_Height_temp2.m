...
Z_SS_order=[];
for ii = length(mybetanew(1:484)):-1:1
...
for ii = 3 :length(wave_data.po)
    ...
    if ii >= 16
        Grid_available_aop(ii-15, :) = actual_range;
    end
end
...
[C,h1] = contourf(P(1:lastpertcare), x(1:484), Grid_to_Plot,20); hold on;
[C,h2] = contour(P(1:lastpertcare), x(1:484), Grid_to_Plot, 20);
axis([0,0.5,0,0.48])
...
ii = 1;cd(dir_save)
figure(ii)
savefig(ii, ['Gold_Eheight_', num2str(ii), '.fig'])
cd([dir_save, '\Images'])
set(gcf,'PaperPositionMode','auto')
print(['Gold_Eheight_', num2str(ii)],'-dpng', '-r300')
ii = 99;cd(dir_save)
figure(ii)
savefig(ii, ['Gold_Eheight_', num2str(ii), '.fig'])
cd([dir_save, '\Images'])
