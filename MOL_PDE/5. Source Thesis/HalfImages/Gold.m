...
%% Run Simulation 0D
    ...
    cd([dir_save]) 
    save('Data0D_Gold', 'Z0D','Y0D','V0D', 't', 'x', 'mybeta', 'runtime')
...
cd(dir_source);
    ...
    cd([dir_save]) 
    save('DataFD_Gold', 'Z0D','Y0D','V0D', 't', 'x', 'mybeta', 'runtime')
...
for ii = 1:NumberOfFig
    savefig(ii, ['Gold', num2str(ii), 'Half.fig'])
end
...
for i = 1:NumberOfFig
    ...
    print(['Gold', num2str(i),'Half'],'-dpng', '-r300')
end
