function [ dydt ] = odefun_Koe( t, y , mybeta, Diff_type, D)
...
%% Split into Z, V, Y
M = length(y)/5;
Z = y(1:M);    Y = y(M+1:2*M);    V = y(2*M+1:3*M); N = y(3*M+1:4*M); I = y(4*M+1:5*M);
...
[L_Z, L_Y, L_V, L_N, L_I] = calc_Koe(Z, Y, V, N, I, mybeta);
...
elseif Diff_type == 2
    ...
    d2Zdx2 = D/dx^2*(term1 + mini*(term2 + term3));
else
...
dZdt = d2Zdx2 +L_Z;
dYdt = L_Y;
dVdt = 1/Cm*(F*V_cyto*lil_z*d2Zdx2) +L_V; % F/100 conversion to nM
dNdt = L_N;
dIdt = L_I;

%% Output form
dydt = [dZdt; dYdt; dVdt; dNdt; dIdt];
